...
class SearchTest(TransactionTestCase):
    ...
    client = Client()
    user_list = [
            ("Umut","Ozdemir",hashlib.sha256("123456".encode("utf-8")).hexdigest(),"umut@gmail.com",hashlib.sha256("umut@gmail.com".encode("utf-8")).hexdigest(),"I am a junior student at Bogazici University and my major degree is in Computer Engineering, also I study a minor degree in Electrics and Electronics Engineering. Finally, I like everything about basketball.",1,"Computer Engineering","Umut"),
            ("Öykü","Yilmaz",hashlib.sha256("1111111".encode("utf-8")).hexdigest(),"oyku@gmail.com",hashlib.sha256("oyku@gmail.com".encode("utf-8")).hexdigest(),"A CmpE student in Bogazici University. Loves to learn new things, swimming, and also doing nothing.",2,"Computer Engineering","Oyku"),
            ("Hasan Ramazan","Yurt",hashlib.sha256("hfaonfsln".encode("utf-8")).hexdigest(),"ramazan@gmail.com",hashlib.sha256("ramazan@gmail.com".encode("utf-8")).hexdigest(),"I am a junior Computer Engineering student at Bogazici University. I can describe myself as creative, sociable and an engineer ready to learn. Also, I am a passionate esports follower.",5,"Computer Engineering","Yurt"),
            ("Ahmet","Dadak",hashlib.sha256("bsfuaıbsbf".encode("utf-8")).hexdigest(),"ahmet@gmail.com",hashlib.sha256("ahmet@gmail.com".encode("utf-8")).hexdigest(),"Junior computer engineering student at Bogazici University. Writes code, scripts, and lyrics. Interested in Web Applications and ML. Interested in sports, literature, design, and music.",8,"Computer Engineering","Ahmet")
    ]
    valid_token = "b429d0dc977130045e56d72e76ef0aae718baa91221f404580900ae60a90a41a"

    def setUp(self):
        if not SearchTest.is_runnable:
            return
        with connection.cursor() as cursor:
            # Create user table
            sql = """CREATE TABLE `users` (`id` INT AUTO_INCREMENT,`name` VARCHAR(30) NOT NULL,`surname` VARCHAR(30) NOT NULL,`password_hash` CHAR(64) NOT NULL,
                                                `e-mail` VARCHAR(255) NOT NULL,`token` VARCHAR(255) NULL,`about_me` TEXT(330) NOT NULL,`job_id` TINYINT NOT NULL,`field_of_study` VARCHAR(50) NOT NULL,
                                                `forget_password_ans` VARCHAR(50) NOT NULL,PRIMARY KEY (`id`),UNIQUE(`e-mail`));"""
            cursor.execute(sql)
            # Add some test users
            sql = 'INSERT INTO `users` (`name`, `surname`, `password_hash`, `e-mail`, `token`, `about_me`, `job_id`, `field_of_study`, `forget_password_ans`) VALUES'
            for test_user in SearchTest.user_list:
                cursor.execute(sql + "('"+"','".join([str(x) for x in test_user])+"');")
        SearchTest.is_runnable = False
    ...
    def test_result_format(self):
        ...
        expected_fields = ['name', 'surname','e-mail', 'about_me', 'job', 'field_of_study']
    ...
    # Test If search function is called with a e-mail in the Database,
    # function returns the user with this e-mail.
    def test_email_search(self):
        ...
        resp = SearchTest.client.get('/api/search/',{"token":SearchTest.valid_token,"search_string":search_string})
        search_result = json.loads(resp.content)
        # Check the e-mail field of the result
        self.assertEqual(search_string,search_result[0]['e-mail'],"It doesn't return the user with the email {}".format(search_string))
