    ...
    private fun setupSearch(view:SearchView){
        ...
        view.setOnQueryTextListener( object: SearchView.OnQueryTextListener{
            ...
            override fun onQueryTextSubmit(query: String?): Boolean {
                (searchRecyclerView.adapter as SearchElementsAdapter).addElement(0, query.toString())
                return Log.println(Log.ERROR, "SEARCH:",(query + searchRecyclerView.adapter?.itemCount.toString()).trim())*0 == 0
            }
        })

        // set close listener
        view.setOnCloseListener {
            // make search options gone
            binding.searchAmongRb.visibility = View.GONE
            // clear search elements
            (searchRecyclerView.adapter as SearchElementsAdapter).clearElements()
